...
export const ProfileFollowerCard = ({ id, name, avatar }: ProfileFollowerCardProps): ReactElement => {
  ...
  useEffect(() => {
    if (!token) return;
    recipesApi
      .getUserRecipes(token, id, { limit: GALLERY_LIMIT })
      .then((res) => {
        setRecipes(res.data);
          ...
          {recipes.map((recipe) => (
            <li key={recipe.id} className={styles.galleryItem}>
              <NavLink to={`/recipe/${recipe.id}`}>
                <img src={recipe.image ?? recipe.thumbnail ?? ""} alt={recipe.title} className={styles.galleryImage} />
              </NavLink>
            </li>
          ))}
        </ul>
      )}
      <NavLink to={`/user/${id}`} className={styles.recipeLink}>

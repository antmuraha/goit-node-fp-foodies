...
export const ProfileContentList = ({ userId, isOwnProfile }: ProfileContentListProps): ReactElement => {
    ...
    useEffect(() => {
        ...
        switch (tab) {
            case TabsListTab.RECIPES: recipes.loadOwnRecipes(); break;
            case TabsListTab.FAVORITES: if (isOwnProfile) favorites.loadFavorites(); break;
            case TabsListTab.FOLLOWERS: followers.loadFollowers(); break;
            case TabsListTab.FOLLOWING: if (isOwnProfile) following.loadFollowing(); break;
        }
    }, [tab, page]);
